...
**우리 팀용으로 수정했습니다.** 짧은 게 곧 좋은 건 아니므로 `ponytail` 에 지역
수정 조항을 붙였습니다: 사다리는 여전히 **필수**지만, 짧음은 **이미 올바른
선택지들 사이의 동점 처리 기준**이지 정확성과 맞바꿀 대상이 아닙니다. 그 앞의
추론은 분량 예산에 포함되지 않으며, 경계 조건·에러 처리·안전성·가독성을 얻는
경우 **분량 초과가 허용**됩니다. 경계에서 틀린 짧은 답은 줄 수만 적은 버그입니다.
군더더기 허용이 아닙니다 — 투기적 추상화와 불필요한 의존성은 여전히 거부됩니다.
