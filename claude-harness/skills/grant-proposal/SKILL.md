---
name: grant-proposal
description: "Draft a structured grant proposal — KAKENHI, NSF, NSFC, ERC, DFG, SNSF, ARC, NWO, or generic. Use when user says \"write grant\", \"grant proposal\", \"申請書\", \"write KAKENHI\", \"科研費\", \"基金申请\", \"写基金\", \"NSF proposal\", or wants to turn research ideas into a funding application."
...
> ⚠️ **Codex MCP fallback**: If `mcp__codex__codex` is not available (no OpenAI API key), skip external review. Note "External review skipped — no Codex MCP available. Consider running `/auto-review-loop — reviewer: llm` separately." in GRANT_REVIEW.md. The proposal is still usable without external review.
