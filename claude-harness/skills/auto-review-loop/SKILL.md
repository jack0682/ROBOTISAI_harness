...
description: Autonomous multi-round review loop with a selectable reviewer backend (REVIEWER_BACKEND: codex, manual, LLM, MiniMax). Use when user says \"auto review loop\", \"review until it passes\", \"llm review\", \"minimax review\".
argument-hint: [topic-or-scope]
allowed-tools: Bash(*), Read, Grep, Glob, Write, Edit, Skill, mcp__codex__codex, mcp__codex__codex-reply, mcp__manual_review__review, mcp__manual_review__review_reply, mcp__llm-chat__chat, mcp__minimax-chat__minimax_chat
---
...
- **REVIEWER_BACKEND = `codex`** — Default: Codex MCP (xhigh). Override with `— reviewer: oracle-pro` for Oracle MCP, `— reviewer: manual` for Manual Review MCP, `— reviewer: llm` for any OpenAI-compatible API (via the `llm-chat` MCP server), or `— reviewer: minimax` for the MiniMax API. If manual-review MCP is unavailable, stop and print the install command; do not fall back to Codex. See `shared-references/reviewer-routing.md` and the back-end configs under "Reviewer Calling Convention". (Note: `codex`/`oracle`/`manual` are threaded reviewers carrying round-to-round memory; `llm`/`minimax` are stateless per-round calls — for those, round memory is carried by including the prior round's summary in each review prompt, never by `threadId`.)
...
**If REVIEWER_BACKEND = `llm`** (any OpenAI-compatible API — stateless per round):
  Configure the `llm-chat` MCP server in `~/.claude/settings.json`:
  ```json
  { "mcpServers": { "llm-chat": { "command": "/usr/bin/python3",
      "args": ["~/.claude/mcp-servers/llm-chat/server.py"],
      "env": { "LLM_API_KEY": "...", "LLM_BASE_URL": "https://api.deepseek.com/v1",
               "LLM_MODEL": "deepseek-chat" } } } }
  ```
  Providers (LLM_BASE_URL / LLM_MODEL): OpenAI `…/v1` `gpt-4o`,`o3` · DeepSeek
  `https://api.deepseek.com/v1` `deepseek-chat`,`deepseek-reasoner` · MiniMax
  `https://api.minimax.io/v1` `MiniMax-M2.7` · Kimi `https://api.moonshot.cn/v1` ·
  GLM `https://open.bigmodel.cn/api/paas/v4` · SiliconFlow / DashScope / 零一万物.
  Call: `mcp__llm-chat__chat` with `prompt`, `model`, `system: "You are a senior ML reviewer..."`.
  Fallback (no MCP): `curl -s "${LLM_BASE_URL}/chat/completions" -H "Authorization: Bearer ${LLM_API_KEY}" -d '{"model":"${LLM_MODEL}","messages":[{"role":"system",...},{"role":"user",...}],"max_tokens":4096}'`.

**If REVIEWER_BACKEND = `minimax`** (MiniMax API — stateless per round; equivalent
  to `llm` pinned to MiniMax):
  Primary: `mcp__minimax-chat__minimax_chat` with `prompt`, `model: "MiniMax-M2.7"`,
  `system: "You are a senior ML researcher..."`.
  Fallback: `curl -s "https://api.minimax.io/v1/chat/completions" -H "Authorization: Bearer $MINIMAX_API_KEY" -d '{"model":"MiniMax-M2.7","messages":[...],"max_tokens":4096}'`
  (key from `~/.claude/settings.json` `env.MINIMAX_API_KEY` or environment).
  Why a dedicated path: Codex CLI uses OpenAI's Responses API (`/v1/responses`),
  unsupported by third-party providers, so MiniMax is called directly.

Prompt fidelity: every backend receives exactly the same review prompt text.
For stateless backends (`llm`/`minimax`), prepend the prior round's summary to the
prompt to carry round-to-round memory. Review tracing applies to all backends.
