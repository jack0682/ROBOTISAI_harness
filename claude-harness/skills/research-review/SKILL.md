...
Branch on `REVIEWER_BACKEND` per
[`shared-references/reviewer-routing.md`](../shared-references/reviewer-routing.md)
— it is the canonical routing contract for every backend (`codex`, `oracle-pro`,
`gemini`, `manual`), including the exact MCP call shapes, thread continuity for
follow-up rounds, and what each backend does when it is not installed.

Two rules that this skill must not soften, restated because they are the ones
most often lost at the call site: the prompt the reviewer receives is *exactly*
the text Codex would have received, and review tracing
([`review-tracing.md`](../shared-references/review-tracing.md)) applies to every
backend equally.
