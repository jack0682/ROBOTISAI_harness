---
name: training-check
description: Watch WandB metrics during training for NaN, loss divergence and idle GPUs, so a broken run does not burn GPU hours. Use when user says "is training healthy", "학습 잘 되고 있나", "loss가 이상해", "NaN", "check wandb".
