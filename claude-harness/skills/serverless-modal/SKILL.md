...
description: "The Modal provider — GPU workloads on Modal, no SSH, no Docker, scale-to-zero. Normally reached through `/run-experiment`; call it directly when the work is Modal-specific. Use when user says \"modal run\", \"modal training\", \"run on modal\", \"deploy to modal\", \"serverless GPU\"."
