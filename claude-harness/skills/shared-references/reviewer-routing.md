...
## Optional: Gemini (third model family, local wrapper)

When the user explicitly passes `— reviewer: gemini`, route the review through the
**`gemini-review` MCP server** — a stdlib-only stdio wrapper around the local
`gemini` CLI, maintained in-harness at
`mcp-servers/gemini-review/server.py`. Gemini is a *third* non-Claude family
alongside the GPT reviewers (codex / oracle); running a GPT reviewer **and** a
Gemini reviewer on the same artifact gives two independent vantages, which is the
strongest configuration for high-stakes theory/proof verification
(`acceptance-gate.md`, `kernel/verification_authority.md`).

### Routing Logic

```
If `— reviewer: gemini`:
    → Check if the gemini-review `review` tool is available
        (server name `gemini-review`; tool `review`).
    → If available:
        Call it with:
          prompt: [role + objective + desired output — raw, un-leading]
          files:  [absolute paths the reviewer reads directly]
          model:  optional (omit → gemini CLI default)
    → If NOT available:
        Print: "⚠️ gemini-review MCP not installed. Install with:
                claude mcp add gemini-review -s user -- python3 \
                <harness>/mcp-servers/gemini-review/server.py"
        Fall back to Codex xhigh.
```

### Invariants

- `— reviewer: gemini` ONLY takes effect when explicitly passed.
- Reviewer-independence still applies: pass file paths + objective, never a
  Claude-side summary or recommendation. The wrapper forwards raw content and
  returns Gemini's verdict unaltered.
- Gemini is non-Claude, so it satisfies a Type-B cross-family acceptance gate on
  its own; for a stronger verdict, pair it with a GPT reviewer and treat a
  disagreement as a signal to dig, not average.
- Latency ~20s+ per call (CLI-backed) — fine for one-shot or final-round review;
  prefer codex for tight multi-round loops.

### NOT installed = graceful fallback

If `gemini-review` is not registered, `— reviewer: gemini` prints the install
command and falls back to Codex. No breakage.
...
> **Platform note**: Manual review requires MCP tools (available only in Claude Code). Mirrored skill packs under `codex-skills/skills-codex/` and `codex-skills/skills-codex-*-review/` do NOT include manual-review wiring — they target Codex CLI and other platforms that lack MCP support. Oracle-pro support in those mirrors is unaffected.
