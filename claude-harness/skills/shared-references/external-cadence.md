...
- **Daily literature watch.** A once-a-day `/research-lit` or
  `/arxiv` sweep for new arXiv papers in a tracked direction. The
...
- `/auto-review-loop — reviewer: llm` and `— reviewer: minimax` — the same loop
  on an alternate reviewer backend (they were standalone skills once; the
  backend is a switch now); same internal round cadence, but each round's
  prior-round summary is fed into the next prompt — a stateless per-round API
  call, not a shared thread, and still verdict-bearing and self-iterating
...
> **Adjacent but distinct — `dse-loop` (retired).** It also looped internally, so do
