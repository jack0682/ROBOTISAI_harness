...
> **Kernel owns this principle.** "What you persist becomes tomorrow's fact —
> never store transient state, a one-off failure, or a negative self-capability
> claim as durable fact; store the fix or the class-level rule instead" is kernel
> law in `kernel/uncertainty_protocol.md`. This file is the ARIS *mechanism*
> (capture_filter, the do-NOT-capture classes). Under the harness, defer to the
> kernel for the principle.
