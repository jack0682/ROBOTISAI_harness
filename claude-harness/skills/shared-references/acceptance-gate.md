...
> **Kernel owns this principle.** The drive-vs-acquit asymmetry — a force that
> wants work done may drive or reject it but never *acquit* it; quality verdicts
> need a check independent of what produced the work; N same-family runs are not a
> jury — is kernel law in `kernel/verification_authority.md`. This file is the
> ARIS *mechanism* (Type-A/B gate taxonomy, cross-model routing). Under the
> harness, defer to the kernel for the principle; do not restate it.
...
the `/experiment-bridge` auto-debug cycle, the
...
|---|---|---|---|---|
| `dse-loop` (retired) | objective metric converged / TIMEOUT / PATIENCE | A | benchmark harness emits the number; Claude reads & compares to budget | ✅ safe same-model |
...
> `dse-loop` is **retired** — kept here because it is the cleanest example of
> the distinction, not because it can be invoked. The same caveat applies to any
> loop whose stop gate is a number the harness itself produces.
