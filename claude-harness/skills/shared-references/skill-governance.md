...
> **Kernel owns this principle.** "Automated rewriting of human-authored artifacts
> is held to the same guard as any irreversible action; the recorded change is
> what authorizes it" is kernel law in `kernel/operating_principles.md` (§5), and
> the author≠acquitter rule is in `kernel/verification_authority.md`. This file is
> the ARIS *mechanism* (provenance.json, assert_cross_family, content_hash). Under
> the harness, defer to the kernel for the principle.
