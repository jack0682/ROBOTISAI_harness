...
description: "Generate publication-quality AI illustrations and architecture diagrams, with a selectable image backend (IMAGE_BACKEND). Use when user says \"生成图表\", \"画架构图\", \"AI绘图\", \"paper illustration\", \"generate diagram\", \"codex image\", or needs visual figures for papers."
argument-hint: "[description-or-method-file] [— style-ref: <source>] [— backend: gemini|codex-image2]"
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, mcp__codex__codex, mcp__codex__codex-reply, WebSearch, mcp__codex-image2__generate, mcp__codex-image2__generate_start, mcp__codex-image2__generate_status
---
...
- **IMAGE_BACKEND = `gemini`** — Image renderer. Default `gemini` (below). Override
  with `— backend: codex-image2` for the experimental local Codex app-server bridge
  (see "Alternative renderer" near the end). The plan → optimize → generate → review
  → refine workflow is identical; only the Step-2/Step-generate calls differ.
...
- **API_KEY_ENV = `GEMINI_API_KEY`** — Environment variable (gemini backend)
...
## Alternative renderer: `codex-image2` (experimental)

When `— backend: codex-image2` is passed (or the user asks for "codex image"),
swap **only the rendering call** — Steps 1–3 (plan/layout/style) and Steps 5–7
(review/refine/finalize) are unchanged. This path uses Codex-native image
generation through a local Codex app-server bridge instead of Gemini. It was
formerly the standalone `paper-illustration-image2` skill, merged here.

Backend constants (override the gemini ones):
- **RENDERER = `codex-image2`**, **NATIVE_IMAGE_REQUIREMENT = `strict`** — accept
  only native `imageGeneration` output; reject shell/Python fallbacks.
- **OPTIONAL_TEXT_CRITIC = `mcp__codex__codex`** — optional text-only layout/style
  second opinion. MAX_ITERATIONS/TARGET_SCORE/OUTPUT_DIR as above.

Generate (replaces Step 4):
1. `mcp__codex-image2__generate_start` with `prompt` (final image prompt), `cwd`
   (project/paper root), `outputPath: figures/ai_generated/figure_v1.png`,
   `system: "Academic paper figure. Prefer crisp English labels."`,
   `timeoutSeconds: 180`.
2. Poll `mcp__codex-image2__generate_status` with bounded waits until `done=true`
   with `status` ∈ {`completed`, `failed`}. On failure, report the bridge error
   directly — never hide it or silently fall back.

Bundled helper: `scripts/paper_illustration_image2.py` (resolved self-contained via
`$CLAUDE_SKILL_DIR/scripts/paper_illustration_image2.py`; legacy `.aris/tools/` and
`tools/` resolver layers also honored). Invoke as `python3 "$IMAGE2_HELPER" <subcommand>`.

Requires the `codex-image2` MCP bridge to be configured; if absent, stop and say so
(do not fall back to gemini unless the user asks).
